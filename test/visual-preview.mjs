...
try {
    ...
    const waystationFidelity = await desktop.locator('.tf-home-page.is-forum-waystation').evaluate(node => {
        ...
        return {
            ...
            firstPostUsesSinglePaperLayer: !getComputedStyle(firstPost).backgroundImage.includes('wugang-paper-border.png'),
    ...
    const callsBeforePromptViewer = await desktop.evaluate(() => globalThis.SillyTavern.getContext().generateCalls);
    await desktop.locator('[data-action="open-prompt-viewer"]').first().click();
    await desktop.locator('.tf-prompt-viewer-message').first().waitFor({ state: 'visible' });
    const promptViewerTotal = Number((await desktop.locator('[data-prompt-viewer-total]').innerText()).replace(/[^\d]/g, ''));
    if (!Number.isFinite(promptViewerTotal) || promptViewerTotal <= 0) throw new Error(`prompt viewer did not calculate a positive input token total: ${promptViewerTotal}`);
    if (await desktop.locator('.tf-prompt-viewer-message').count() !== visibleReadOrderIds.length) throw new Error('prompt viewer messages do not match the visible API order');
    if (!await desktop.locator('.tf-prompt-viewer-messages').getByText('我们来到旧码头', { exact: false }).count()) throw new Error('prompt viewer does not expose the complete selected chat context');
    if (await desktop.evaluate(() => globalThis.SillyTavern.getContext().generateCalls) !== callsBeforePromptViewer) throw new Error('opening the prompt viewer unexpectedly called the AI');
    const desktopPromptViewerWidth = await desktop.locator('.tf-prompt-viewer').evaluate(node => ({ scroll: node.scrollWidth, client: node.clientWidth }));
    if (desktopPromptViewerWidth.scroll > desktopPromptViewerWidth.client + 2) throw new Error(`desktop prompt viewer overflowed horizontally (${desktopPromptViewerWidth.scroll} > ${desktopPromptViewerWidth.client})`);
    await desktop.locator('[data-action="close-prompt-viewer"]').click();
    ...
    const mobileCallsBeforePromptViewer = await mobile.evaluate(() => globalThis.SillyTavern.getContext().generateCalls);
    await mobile.locator('[data-action="open-prompt-viewer"]').first().click();
    await mobile.locator('.tf-prompt-viewer-message').first().waitFor({ state: 'visible' });
    const mobilePromptViewerWidth = await mobile.locator('.tf-prompt-viewer').evaluate(node => ({ scroll: node.scrollWidth, client: node.clientWidth }));
    if (mobilePromptViewerWidth.scroll > mobilePromptViewerWidth.client + 2) throw new Error(`mobile prompt viewer overflowed horizontally (${mobilePromptViewerWidth.scroll} > ${mobilePromptViewerWidth.client})`);
    if (await mobile.evaluate(() => globalThis.SillyTavern.getContext().generateCalls) !== mobileCallsBeforePromptViewer) throw new Error('mobile prompt viewer unexpectedly called the AI');
    await mobile.locator('[data-action="close-prompt-viewer"]').click();
