...
const COMPANION_DEVICE_SKINS = Object.freeze([
    { id: 'classic', name: '经典蛋机', note: '珠光搪瓷 · 复古绿屏', brand: 'POCKET TRAVELER' },
    { id: 'pocket', name: '口袋掌机', note: '肩角机身 · 雾蓝面板', brand: 'GAMEFIELD JR.' },
    { id: 'crystal', name: '透明糖果机', note: '可见内构 · 冰蓝光屏', brand: 'CLEARLINK 02' },
    { id: 'arcane', name: '魔法通讯器', note: '鎏金符文 · 紫晶视窗', brand: 'FAMILIAR LINK' },
    { id: 'terminal', name: '机械终端', note: '铆接装甲 · 琥珀雷达', brand: 'FIELD UNIT 07' },
    { id: 'camp', name: '露营电台', note: '木纹护角 · 苔绿荧屏', brand: 'TRAIL RADIO' },
    { id: 'marine', name: '深海观察窗', note: '黄铜舷窗 · 海蓝声呐', brand: 'ABYSS WATCH' },
    { id: 'arcade', name: '迷你街机', note: '霓虹边灯 · 靛青像素屏', brand: 'STAR ARCADE' },
    { id: 'lunar', name: '月相怀表', note: '星轨表圈 · 月白夜屏', brand: 'LUNAR PHASE' },
    { id: 'berry', name: '莓果翻盖机', note: '双色折叠 · 莓粉亮屏', brand: 'BERRY FLIP' },
]);
...
const COMPANION_HABITATS = Object.freeze([
    { id: 'meadow', name: '风吹草地', symbol: '≋', note: '柔软草坡' },
    { id: 'pond', name: '荷叶池塘', symbol: '≈', note: '水光与荷叶' },
    { id: 'bedroom', name: '暖灯卧室', symbol: '▤', note: '安静暖光' },
    { id: 'forest', name: '林间树屋', symbol: '♧', note: '枝叶木屋' },
    { id: 'snowfield', name: '雪原营地', symbol: '·', note: '雪地帐篷' },
    { id: 'city', name: '城市天台', symbol: '▥', note: '城市灯火' },
    { id: 'space', name: '星际舷窗', symbol: '✦', note: '缓慢星轨' },
    { id: 'arcade', name: '像素街机厅', symbol: '▦', note: '霓虹格栅' },
]);
const COMPANION_WEATHER_OPTIONS = Object.freeze([
    { id: 'auto', name: '自动', symbol: '◌', note: '跟随本地' },
    { id: 'sunny', name: '晴朗', symbol: '☀', note: '暖光' },
    { id: 'cloudy', name: '多云', symbol: '☁', note: '柔云' },
    { id: 'rain', name: '小雨', symbol: '╱', note: '细雨' },
    { id: 'wind', name: '微风', symbol: '≋', note: '风线' },
    { id: 'snow', name: '飘雪', symbol: '·', note: '雪粒' },
]);
const COMPANION_TIME_OPTIONS = Object.freeze([
    { id: 'auto', name: '自动', symbol: '◷', note: '本地时间' },
    { id: 'dawn', name: '清晨', symbol: '◒', note: '晨光' },
    { id: 'day', name: '白天', symbol: '○', note: '日光' },
    { id: 'dusk', name: '黄昏', symbol: '◐', note: '暮色' },
    { id: 'night', name: '夜晚', symbol: '☾', note: '月光' },
]);
...
const viewState = {
    ...
    companionActionTimer: 0,
    companionActionToken: 0,
    fortuneRevealChoice: '',
    fortuneAiMode: false,
...
function renderModuleCard(definition) {
    ...
    const modeControl = definition.id === 'forum'
        ? '<span class="tf-mode-note">论坛随“刷新”生成</span>'
        : `<label class="tf-module-mode"><span>生成方式</span><select data-module-field="generationMode"><option value="linked" ${module.generationMode === 'linked' ? 'selected' : ''}>持续联动 · 随论坛刷新判定</option><option value="independent" ${independent ? 'selected' : ''}>${definition.id === 'travel' ? '独立出发 · 仅主动出发时调用' : '独立生成 · 使用本模块 API'}</option>${definition.id === 'fortune' ? `<option value="local" ${module.generationMode === 'local' ? 'selected' : ''}>本地随机 · 不调用 API</option>` : ''}</select><small>${module.generationMode === 'linked' ? '每轮先按下方概率判定；命中后才与论坛共用本次 API，不保证百分百发生。' : module.generationMode === 'local' ? '按世界内日期缓存，同一天不会重复调用 API。' : definition.id === 'travel' ? '主动出发时预生成完整行程；途中按本地时间释放，不重复调用 API。' : '手动或自动单独生成；使用下面选择的独立 API。'}</small></label>`;
    ...
    const probability = definition.id === 'forum' ? '' : `<label><span>每轮触发概率（%）</span><input type="number" min="0" max="100" data-module-field="probability" value="${Number(module.probability ?? 35)}"></label><label><span>触发冷却（分钟）</span><input type="number" min="0" max="43200" data-module-field="cooldownMinutes" value="${Number(module.cooldownMinutes || 0)}"></label>`;
...
function getCompanionVisualAction(companion) {
    return viewState.companionVisualAction || (companion?.status === 'resting' ? 'rest' : 'idle');
}

function activateCompanionVisualAction(action) {
    if (viewState.companionActionTimer) globalThis.clearTimeout(viewState.companionActionTimer);
    viewState.companionActionTimer = 0;
    viewState.companionActionToken += 1;
    viewState.companionVisualAction = action === 'rest' ? '' : action;
    const duration = Number(COMPANION_ACTION_DURATIONS[action] || 0);
    if (!duration) return;
    const token = viewState.companionActionToken;
    viewState.companionActionTimer = globalThis.setTimeout(() => {
        if (viewState.companionActionToken !== token) return;
        viewState.companionVisualAction = '';
        viewState.companionActionTimer = 0;
        if (getRoot()?.querySelector('.tf-companion-page')) render({ preserveScroll: true });
    }, duration);
}
...
function renderCompanionAppV2(data) {
    const companion = data.world.companion;
    const visualAction = getCompanionVisualAction(companion);
...
function renderCompanionAppV3(data) {
    ...
    const deviceOptions = COMPANION_DEVICE_SKINS.map(item => `<button type="button" class="tf-device-skin-option ${skin.id === item.id ? 'is-selected' : ''}" data-action="choose-companion-device" data-device-skin="${item.id}" aria-pressed="${skin.id === item.id}" title="${escapeHtml(item.name)} · ${escapeHtml(item.note)}"><span class="tf-device-swatch is-${item.id}" aria-hidden="true"><i></i><b></b><em></em><small></small></span><div><b>${item.name}</b><small>${item.note}</small></div></button>`).join('');
    const renderEnvironmentGroup = (field, label, kicker, options, current) => {
        const selected = options.find(item => item.id === current) || options[0];
        const cards = options.map(item => `<button type="button" class="tf-environment-option is-${field}-${item.id} ${selected.id === item.id ? 'is-selected' : ''}" data-action="choose-companion-environment" data-environment-field="${field}" data-environment-value="${item.id}" role="option" aria-selected="${selected.id === item.id}"><i class="tf-environment-preview" aria-hidden="true"><span>${item.symbol}</span></i><span><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.note)}</small></span><em aria-hidden="true">✓</em></button>`).join('');
        const nativeOptions = options.map(item => `<option value="${item.id}" ${selected.id === item.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`).join('');
        return `<section class="tf-environment-group is-${field}"><header><div><small>${kicker}</small><b>${label}</b></div><span>${selected.symbol} ${escapeHtml(selected.name)}</span></header><div class="tf-environment-options" role="listbox" aria-label="${label}">${cards}</div><select class="tf-environment-native" data-companion-environment="${field}" aria-label="${label}" tabindex="-1">${nativeOptions}</select></section>`;
    };
    const environmentControls = `<div class="tf-pet-environment-controls"><header class="tf-environment-title"><div><small>ROOM & WEATHER</small><b>小窝环境</b></div><p>背景、天气和时间都会立即反映在掌机屏幕中。</p></header>${renderEnvironmentGroup('habitat', '小窝背景', 'SCENE', COMPANION_HABITATS, companion.habitat || 'meadow')}${renderEnvironmentGroup('weather', '小窝天气', 'WEATHER', COMPANION_WEATHER_OPTIONS, companion.weather || 'auto')}${renderEnvironmentGroup('timeOfDay', '小窝时间', 'TIME', COMPANION_TIME_OPTIONS, companion.timeOfDay || 'auto')}</div>`;
    const stat = (label, value) => `<div><span>${label}<b>${Number(value)}</b></span><progress max="100" value="${Number(value)}"></progress></div>`;
...
function renderSourcesSettings() {
    ...
    const sourceSection = `<section class="tf-card tf-settings-card">
        ...
        <div class="tf-generation-ranges"><div><b>每轮帖子数量</b><label>最少<input type="number" data-setting="generation.postsMin" value="${Number(settings.generation.postsMin)}" min="1" max="10"></label><label>最多<input type="number" data-setting="generation.postsMax" value="${Number(settings.generation.postsMax)}" min="1" max="10"></label></div><div><b>每篇初始评论</b><label>最少<input type="number" data-setting="generation.commentsMin" value="${Number(settings.generation.commentsMin)}" min="0" max="15"></label><label>最多<input type="number" data-setting="generation.commentsMax" value="${Number(settings.generation.commentsMax)}" min="0" max="15"></label></div><div><b>回帖后的 AI 跟帖</b><label>最少<input type="number" data-setting="generation.repliesMin" value="${Number(settings.generation.repliesMin)}" min="1" max="8"></label><label>最多<input type="number" data-setting="generation.repliesMax" value="${Number(settings.generation.repliesMax)}" min="1" max="8"></label></div></div><div class="tf-source-order-link"><span>${icon('list')}<b>发送顺序由“论坛设定”统一管理</b><small>这里仅决定哪些来源允许读取，不再保存另一套排序。</small></span><button class="tf-secondary-button" data-action="open-settings" data-section="prompts">管理发送顺序</button></div>
...
async function runGeneration({ automatic = false } = {}) {
    ...
    try {
        ...
        const commentsMaximum = Math.max(0, Math.min(15, Number(settings.generation.commentsMax ?? 3)));
...
async function handleRootClick(event) {
    ...
    if (action === 'set-forum-layout') {
        ...
        if (layout === 'waystation' && getSettings().appearance.forumLayout !== 'waystation') {
            const password = window.prompt('“驿站手账”暂时处于锁定状态。请输入访问密码：')?.trim();
            if (password !== '233') {
                if (password !== undefined) notify('warning', '密码错误，未切换到驿站手账');
                return;
            }
        }
    ...
    if (action === 'companion-feed-food') {
        ...
        activateCompanionVisualAction('feed');
        viewState.companionFoodMenuOpen = false;
        await saveForumData(data, true);
        syncInjection();
        return render({ preserveScroll: true });
    ...
    if (action === 'choose-companion-environment') {
        const field = target.dataset.environmentField;
        const value = target.dataset.environmentValue;
        const select = getRoot()?.querySelector(`[data-companion-environment="${field}"]`);
    ...
    if (action === 'companion-care') {
        ...
        activateCompanionVisualAction(care);
        await saveForumData(data, true);
        syncInjection();
        return render({ preserveScroll: true });
    }
